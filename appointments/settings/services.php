...
if(isset($_GET['action']) && $_GET['action']=='delete'){	
	$db->query_delete("service_time","service_type=1 and service_id='".$_REQUEST['id'] ."'");	
	$db->query_delete("salon_services","service_type=1 and service_id='".$_REQUEST['id'] ."'");
	$db->query_delete("staff_salon_services","service_type=1 and service_id='".$_REQUEST['id'] ."'");
	$db->query_delete("service_spa_service","service_type=1 and service_id='".$_REQUEST['id'] ."'");	
	$db->query_delete("service","id='".$_REQUEST['id'] ."'");	
	$_SESSION['msg']="Your selected service deleted!";
	$general_func->header_redirect($_REQUEST['url']);
} 

$keyword="Search by service name, grooming type";
...
if(isset($_REQUEST['keyword']) && trim($_REQUEST['keyword']) != NULL){
	...
	$query .=" and (service_name LIKE '%" .$_REQUEST['keyword']. "%' ";	
	$query .=" OR name LIKE '%" .$_REQUEST['keyword']. "%' ";
...
(min-device-width: 768px) and (max-device-width: 1024px)  {
	...
	Label the data
	*/
	td:nth-of-type(1):before { content: "Pet Type"; }
...
function show_services(value){
	...
	var ajaxURL="search.php?keywords=" + document.frmsearch.keyword.value +"&type=servicessearch&t=" + Math.random();
...
function del(id,url,name){
	var a=confirm("Are you sure, you want to delete service: '" + name +"'? and all its related record")
    if (a){
 ...
 <div class="breadcrumb">
      	<p><a href="settings/services.php">Services</a> &raquo; Service List</p>
</div>
...
<?php } ?>
 <div class="pageTopPnl">
        	<div class="searchPnl">
				...
				<?php }	?>
        		<input name="keyword"  value="" type="text"  onkeyup="show_services(this.value)" placeholder="Search by service name, grooming type" />
        		<input name="submit" id="submit"  style="display: none;" type="button" value="Clear Search" onclick="location.href='<?=$_SERVER['PHP_SELF']?>'" />
        		 </form>
        		</div>
            <div class="buttonPnl"><input name="add_services" type="button" value="Add Services" onclick="location.href='<?=$general_func->admin_url?>settings/services-new.php?return_url=<?=urlencode($url)?>'" /> 
       </div>
        </div>
