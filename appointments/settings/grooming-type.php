...
if($_SESSION['admin_access_level'] != 1 && ! in_array(13,$_SESSION['access_permission'])){	
...
if(isset($_GET['action']) && $_GET['action']=='delete'){
	...
	$db->query_delete("pets","grooming_id='".$_REQUEST['id'] ."'");	
	$db->query_delete("salon_grooming","grooming_id='".$_REQUEST['id'] ."'");	
	$db->query_delete("service","grooming_id='".$_REQUEST['id'] ."'");	
			
	$db->query_delete("grooming","id='".$_REQUEST['id'] ."'");	
	$_SESSION['msg']="Your selected grooming type deleted!";
	$general_func->header_redirect($_REQUEST['url']);
} 


if(isset($_REQUEST['action']) && $_REQUEST['action']=="EDIT"){
	$sql="select * from grooming where id=" . (int) $_REQUEST['id'] . " limit 1";
...
if(isset($_POST['enter']) && $_POST['enter']=="grooming"){
	$name=trim($_REQUEST['name']);
	
	
	if($_POST['submit']=="Add New"){		
		if($db->already_exist_inset("grooming","name",$name)){
			$_SESSION['msg']="Sorry, your specified grooming type is already taken!";			
		}else{
			$data['name']=$name;		
			$db->query_insert("grooming",$data);
			
			$_SESSION['msg']="Grooming type successfully added!";
			$general_func->header_redirect($_SERVER['PHP_SELF']);
		}
	}else{
		if($db->already_exist_update("grooming","id",$_REQUEST['id'],"name",$name)){	
			$_SESSION['msg']="Sorry, your specified grooming type is already taken!";					
		}else{
			$data['name']=$name;	
			$db->query_update("grooming",$data,"id='".$_REQUEST['id'] ."'");
			
			if($db->affected_rows > 0)
				$_SESSION['msg']="Grooming type successfully updated!";
...
$sql="select * from grooming $query order by $order_by";
...
function del(id,url,name){
	var a=confirm("Are you sure, you want to delete grooming type : '" + name +"' and all related data of it?")
    if (a){
...
function validate(){	
	...
	if(!validate_text(document.ff.name,1,"Please enter grooming type"))
		return false;
		
}		 
</script>
 <div class="breadcrumb">
      	<p><a>Settings</a> &raquo; Grooming Types</p>
</div>
            ...
            <tbody>
             <?php for($j=0; $j<$total_customers; $j++){?>
                ...
                <td style="text-align:center">
                	<img src="images/edit.png" onclick="location.href='<?=$general_func->admin_url?>settings/grooming-type.php?id=<?=$result[$j]['id']?>&action=EDIT&return_url=<?=urlencode($url)?>'" style="cursor:pointer;"  title="EDIT" alt="EDIT" />&nbsp;&nbsp;&nbsp;
