...
if(isset($_GET['action']) && $_GET['action']=='delete'){		
	$db->query_delete("breed","id='".$_REQUEST['id'] ."'");	
	$_SESSION['msg']="Your selected breed deleted!";
	$general_func->header_redirect($_REQUEST['url']);
} 


if(isset($_REQUEST['action']) && $_REQUEST['action']=="EDIT"){
	$sql="select * from breed where id=" . (int) $_REQUEST['id'] . " limit 1";
...
if(isset($_POST['enter']) && $_POST['enter']=="breed"){
	$name=trim($_REQUEST['name']);
	
	
	if($_POST['submit']=="Add New"){		
		if($db->already_exist_inset("breed","name",$name)){
			$_SESSION['msg']="Sorry, your specified breed name is already taken!";			
		}else{
			$data['name']=$name;		
			$db->query_insert("breed",$data);
			
			$_SESSION['msg']="Breed name successfully added!";
			$general_func->header_redirect($_SERVER['PHP_SELF']);
		}
	}else{
		if($db->already_exist_update("breed","id",$_REQUEST['id'],"name",$name)){	
			$_SESSION['msg']="Sorry, your specified breed name is already taken!";					
		}else{
			$data['name']=$name;	
			$db->query_update("breed",$data,"id='".$_REQUEST['id'] ."'");
			
			if($db->affected_rows > 0)
				$_SESSION['msg']="Breed successfully updated!";
...
$sql="select * from breed $query order by $order_by";
...
function del(id,url,name){
	var a=confirm("Are you sure, you want to delete breed: '" + name +"'?")
    if (a){
...
function validate(){	
	...
	if(!validate_text(document.ff.name,1,"Please enter breed name"))
		return false;
		
}		 
</script>
 <div class="breadcrumb">
      	<p><a href="customers/pets.php">Pets</a> &raquo; Breed List</p>
</div>
            ...
            <tbody>
             <?php for($j=0; $j<$total_customers; $j++){?>
                ...
                <td style="text-align:center">
                	<img src="images/edit.png" onclick="location.href='<?=$general_func->admin_url?>customers/breed.php?id=<?=$result[$j]['id']?>&action=EDIT&return_url=<?=urlencode($url)?>'" style="cursor:pointer;"  title="EDIT" alt="EDIT" />&nbsp;&nbsp;&nbsp;
