...
if(isset($_GET['action']) && $_GET['action']=='delete'){
	...
	$db->query_delete("product_size","id='".$_REQUEST['id'] ."'");	
	$_SESSION['msg']="Your selected size deleted!";
	$general_func->header_redirect($_REQUEST['url']);
} 


if(isset($_REQUEST['action']) && $_REQUEST['action']=="EDIT"){
	$sql="select * from product_size where id=" . (int) $_REQUEST['id'] . " limit 1";
...
if(isset($_POST['enter']) && $_POST['enter']=="size"){
	$name=trim($_REQUEST['name']);
	
	
	if($_POST['submit']=="Add New"){		
		if($db->already_exist_inset("product_size","name",$name)){
			$_SESSION['msg']="Sorry, your specified size name is already taken!";			
		}else{
			$data['name']=$name;		
			$db->query_insert("product_size",$data);
			
			$_SESSION['msg']="Size successfully added!";
			$general_func->header_redirect($_SERVER['PHP_SELF']);
		}
	}else{
		if($db->already_exist_update("product_size","id",$_REQUEST['id'],"name",$name)){	
			$_SESSION['msg']="Sorry, your specified size name is already taken!";					
		}else{
			$data['name']=$name;	
			$db->query_update("product_size",$data,"id='".$_REQUEST['id'] ."'");
			
			if($db->affected_rows > 0)
				$_SESSION['msg']="Size successfully updated!";
...
$sql="select * from product_size $query order by $order_by";
...
function del(id,url,name){
	var a=confirm("Are you sure, you want to delete size: '" + name +"'?")
    if (a){
...
function validate(){	
	...
	if(!validate_text(document.ff.name,1,"Please enter size name"))
		return false;
		
}		 
</script>
 <div class="breadcrumb">
      	<p><a href="products/products.php">Products</a> &raquo; Size List</p>
</div>
...
<?php } ?>
      ...
      <thead>
            <tr>
                <th width="91%" style="text-align:left">Name</th>                          
            ...
            <tbody>
             <?php for($j=0; $j<$total_customers; $j++){?>
             	<tr>
                <td style="text-align:left"><?=$result[$j]['name']?></td>                
                <td style="text-align:right" align="right";>
                	<img src="images/edit.png" onclick="location.href='<?=$general_func->admin_url?>products/sizes.php?id=<?=$result[$j]['id']?>&action=EDIT&return_url=<?=urlencode($url)?>'" style="cursor:pointer;"  title="EDIT" alt="EDIT" />&nbsp;&nbsp;&nbsp;
