...
if(isset($_GET['action']) && $_GET['action']=='delete'){
	...
	$_SESSION['msg']="Your selected supplier deleted!";
...
if(isset($_POST['enter']) && $_POST['enter']=="supplier"){
	...
	if($_POST['submit']=="Add New"){		
		if($db->already_exist_inset("product_supplier","name",$name)){
			$_SESSION['msg']="Sorry, your specified supplier name is already taken!";			
		}else{
			$data['name']=$name;		
			$db->query_insert("product_supplier",$data);
			
			$_SESSION['msg']="Supplier successfully added!";
			$general_func->header_redirect($_SERVER['PHP_SELF']);
		}
	}else{
		if($db->already_exist_update("product_supplier","id",$_REQUEST['id'],"name",$name)){	
			$_SESSION['msg']="Sorry, your specified supplier name is already taken!";					
		}else{
			$data['name']=$name;	
			...
			if($db->affected_rows > 0)
				$_SESSION['msg']="Supplier successfully updated!";
...
$sql="select * from product_supplier $query order by $order_by";
...
function validate(){	
	...
	if(!validate_text(document.ff.name,1,"Please enter supplier name"))
		return false;
		
}		 
</script>
 <div class="breadcrumb">
      	<p><a href="products/products.php">Products</a> &raquo; Supplier List</p>
</div>
...
<?php } ?>
        ...
        <ul class="tabBtn">
            	<li class="activeTab"><a style="cursor: pointer;">Product Suppliers</a></li>              
 		</ul>
            ...
            <tbody>
             <?php for($j=0; $j<$total_customers; $j++){?>
                ...
                <td style="text-align:center">
                	<img src="images/edit.png" onclick="location.href='<?=$general_func->admin_url?>products/suppliers.php?id=<?=$result[$j]['id']?>&action=EDIT&return_url=<?=urlencode($url)?>'" style="cursor:pointer;"  title="EDIT" alt="EDIT" />&nbsp;&nbsp;&nbsp;
