...
if(isset($_GET['action']) && $_GET['action']=='delete'){
	...
	$db->query_delete("product_make","id='".$_REQUEST['id'] ."'");	
	$_SESSION['msg']="Your selected manufacturer deleted!";
	$general_func->header_redirect($_REQUEST['url']);
} 


if(isset($_REQUEST['action']) && $_REQUEST['action']=="EDIT"){
	$sql="select * from product_make where id=" . (int) $_REQUEST['id'] . " limit 1";
...
if(isset($_POST['enter']) && $_POST['enter']=="manufacturer"){
	$name=trim($_REQUEST['name']);
	
	
	if($_POST['submit']=="Add New"){		
		if($db->already_exist_inset("product_make","name",$name)){
			$_SESSION['msg']="Sorry, your specified manufacturer name is already taken!";			
		}else{
			$data['name']=$name;		
			$db->query_insert("product_make",$data);
			
			$_SESSION['msg']="Manufacturer successfully added!";
			$general_func->header_redirect($_SERVER['PHP_SELF']);
		}
	}else{
		if($db->already_exist_update("product_make","id",$_REQUEST['id'],"name",$name)){	
			$_SESSION['msg']="Sorry, your specified manufacturer name is already taken!";					
		}else{
			$data['name']=$name;	
			$db->query_update("product_make",$data,"id='".$_REQUEST['id'] ."'");
			
			if($db->affected_rows > 0)
				$_SESSION['msg']="Manufacturer successfully updated!";
...
$sql="select * from product_make $query order by $order_by";
...
function del(id,url,name){
	var a=confirm("Are you sure, you want to delete manufacturer: '" + name +"'?")
    if (a){
...
function validate(){	
	...
	if(!validate_text(document.ff.name,1,"Please enter manufacturer name"))
		return false;
		
}		 
</script>
 <div class="breadcrumb">
      	<p><a href="products/products.php">Products</a> &raquo; Manufacturer List</p>
</div>
            ...
            <tbody>
             <?php for($j=0; $j<$total_customers; $j++){?>
                ...
                <td style="text-align:center">
                	<img src="images/edit.png" onclick="location.href='<?=$general_func->admin_url?>products/manufacturers.php?id=<?=$result[$j]['id']?>&action=EDIT&return_url=<?=urlencode($url)?>'" style="cursor:pointer;"  title="EDIT" alt="EDIT" />&nbsp;&nbsp;&nbsp;
