...
if(isset($_GET['action']) && $_GET['action']=='delete'){		
	$db->query_delete("product_colour","id='".$_REQUEST['id'] ."'");	
	$_SESSION['msg']="Your selected colour deleted!";
	$general_func->header_redirect($_REQUEST['url']);
} 


if(isset($_REQUEST['action']) && $_REQUEST['action']=="EDIT"){
	$sql="select * from product_colour where id=" . (int) $_REQUEST['id'] . " limit 1";
...
if(isset($_POST['enter']) && $_POST['enter']=="color"){
	$name=trim($_REQUEST['name']);
	
	
	if($_POST['submit']=="Add New"){		
		if($db->already_exist_inset("product_colour","name",$name)){
			$_SESSION['msg']="Sorry, your specified colour name is already taken!";			
		}else{
			$data['name']=$name;		
			$db->query_insert("product_colour",$data);
			
			$_SESSION['msg']="Colour successfully added!";
			$general_func->header_redirect($_SERVER['PHP_SELF']);
		}
	}else{
		if($db->already_exist_update("product_colour","id",$_REQUEST['id'],"name",$name)){	
			$_SESSION['msg']="Sorry, your specified colour name is already taken!";					
		}else{
			$data['name']=$name;	
			$db->query_update("product_colour",$data,"id='".$_REQUEST['id'] ."'");
			
			if($db->affected_rows > 0)
				$_SESSION['msg']="Colour successfully updated!";
...
$sql="select * from product_colour $query order by $order_by";
...
function del(id,url,name){
	var a=confirm("Are you sure, you want to delete colour: '" + name +"'?")
    if (a){
...
function validate(){	
	...
	if(!validate_text(document.ff.name,1,"Please enter colour name"))
		return false;
		
}		 
</script>
 <div class="breadcrumb">
      	<p><a href="products/products.php">Products</a> &raquo; Color List</p>
</div>
            ...
            <tbody>
             <?php for($j=0; $j<$total_customers; $j++){?>
                ...
                <td style="text-align:center">
                	<img src="images/edit.png" onclick="location.href='<?=$general_func->admin_url?>products/colors.php?id=<?=$result[$j]['id']?>&action=EDIT&return_url=<?=urlencode($url)?>'" style="cursor:pointer;"  title="EDIT" alt="EDIT" />&nbsp;&nbsp;&nbsp;
